...
def main():
    ...
    if script_args.mode == "sft":
        # 加载模型权重
        # ckpt/checkpoint-380000/pytorch_model.bin
    ...
    if script_args.mode == "ptm":
        train_ds = PretrainDataset(data_path_list, max_length = model_args.max_seq_len, memmap=False)
    elif script_args.mode == "sft":
        train_ds = SFTDataset(data_path_list, max_length = model_args.max_seq_len)
    else:
        logger.error("***************NO TRAIN_DS DATA********************")

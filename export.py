...
from model import ModelArgs, TinyLlama2
...
def load_checkpoint(checkpoint):
    """
        ...
        返回：
            - model (TinyLlama2): 初始化并加载了权重的Transformer模型实例
    """
    # load the provided model checkpoint
    checkpoint_dict = torch.load(checkpoint, map_location='cpu')
    gptconf = ModelArgs(**checkpoint_dict['model_args'])
    model = TinyLlama2(gptconf)
...
def load_meta_model(model_path):
    """
        ...
        返回：
            - model (TinyLlama2): 合并后的新Transformer模型实例
    """
...
def load_hf_model(model_path):
    """
        ...
        返回：
            - model (TinyLlama2): 转换后的自定义Transformer模型实例
    """
    try:
